...
namespace BirdGame.Editor
{
    public class GameConfigExporter : EditorWindow
    {
        ...
        private void ExportAllConfigs()
        {
            try
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(savePath)))
                {
                    ...
                    ExportEggProbabilityConfig(package);
        ...
        private void ExportEggProbabilityConfig(ExcelPackage package)
        {
            var worksheet = package.Workbook.Worksheets.Add("鸟蛋开鸟概率");

            // 设置表头
            worksheet.Cells[1, 1].Value = "场景索引";
            worksheet.Cells[1, 2].Value = "鸟蛋索引";
            worksheet.Cells[1, 3].Value = "鸟ID";
            worksheet.Cells[1, 4].Value = "鸟名称";
            worksheet.Cells[1, 5].Value = "权重";
            worksheet.Cells[1, 6].Value = "概率(%)";
            ...
            if (shopConfig != null && shopConfig.sceneEggs != null)
            {
                for (int sceneIndex = 0; sceneIndex < shopConfig.sceneEggs.Count; sceneIndex++)
                {
                    // 建立该场景的 鸟ID -> 名称 映射，避免逐个查找报错
                    var nameById = new Dictionary<int, string>();
                    if (birdConfig != null && sceneIndex < birdConfig.sceneBirds.Count &&
                        birdConfig.sceneBirds[sceneIndex].birdClasses != null)
                    {
                        foreach (var birdClass in birdConfig.sceneBirds[sceneIndex].birdClasses)
                        {
                            if (birdClass?.birds == null) continue;
                            foreach (var bird in birdClass.birds)
                            {
                                if (bird != null)
                                    nameById[bird.id] = birdClass.birdName;
                            }
                        }
                    }

                    var sceneEgg = shopConfig.sceneEggs[sceneIndex];
                    if (sceneEgg.eggs == null) continue;

                    for (int eggIndex = 0; eggIndex < sceneEgg.eggs.Length; eggIndex++)
                    {
                        var egg = sceneEgg.eggs[eggIndex];
                        if (egg.birds == null || egg.birds.Length == 0) continue;

                        float total = egg.GetTotalProbability();
                        foreach (var eggBird in egg.birds)
                        {
                            worksheet.Cells[rowIndex, 1].Value = sceneIndex;
                            worksheet.Cells[rowIndex, 2].Value = eggIndex;
                            worksheet.Cells[rowIndex, 3].Value = eggBird.birdType;
                            worksheet.Cells[rowIndex, 4].Value =
                                nameById.TryGetValue(eggBird.birdType, out var birdName) ? birdName : "未找到";
                            worksheet.Cells[rowIndex, 5].Value = eggBird.probability;
                            worksheet.Cells[rowIndex, 6].Value =
                                total > 0 ? Math.Round(eggBird.probability / total * 100f, 2) : 0;
                            rowIndex++;
                        }
                    }
                }
