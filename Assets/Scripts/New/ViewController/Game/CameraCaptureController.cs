...
namespace BirdGame
{
    ...
    public class CameraCaptureController : ViewControllerBase
    {
        ...
        private IEnumerator CaptureCoroutine()
        {
            ...
            float halfW = frameW * 0.5f;
            float halfH = frameH * 0.5f;
            float cxScreen = Mathf.Clamp(mousePos.x, halfW, Screen.width - halfW);
            float cyScreen = Mathf.Clamp(mousePos.y, halfH, Screen.height - halfH);
            ...
            try
            {
                // 关键：ScreenCapture返回的纹理尺寸可能 != Screen.width/Height（DPI/缩放/分辨率适配）
                // 把屏幕坐标按texture/screen比例缩放到texture坐标系
                int texW = fullScreen.width;
                int texH = fullScreen.height;
                float sx = (float)texW / Mathf.Max(1, Screen.width);
                float sy = (float)texH / Mathf.Max(1, Screen.height);

                int cropW = Mathf.Clamp(Mathf.RoundToInt(frameW * sx), 1, texW);
                int cropH = Mathf.Clamp(Mathf.RoundToInt(frameH * sy), 1, texH);
                int cropX = Mathf.Clamp(Mathf.RoundToInt((cxScreen - halfW) * sx), 0, texW - cropW);
                int cropY = Mathf.Clamp(Mathf.RoundToInt((cyScreen - halfH) * sy), 0, texH - cropH);

                Debug.Log($"[Camera] Screen={Screen.width}x{Screen.height} TexCap={texW}x{texH} " +
                          $"Frame={frameW}x{frameH} CropRect=({cropX},{cropY},{cropW},{cropH})");

                Color[] pixels = fullScreen.GetPixels(cropX, cropY, cropW, cropH);
