...
namespace BirdGame
{
    ...
    public class PhotoPopup : UIBase
    {
        ...
        private static bool TryCopyImageToClipboard(Texture2D tex)
        {
            ...
            // macOS: 先写临时PNG，再用osascript把图片塞进剪贴板
            try
            {
                string tempPath = System.IO.Path.Combine(
                    System.IO.Path.GetTempPath(),
                    $"featherland_clip_{System.DateTime.Now:yyyyMMddHHmmssfff}.png");
                System.IO.File.WriteAllBytes(tempPath, tex.EncodeToPNG());

                // AppleScript：把PNG文件的字节读进剪贴板（«class PNGf»是PNG格式的OSType code）
                string script = $"set the clipboard to (read (POSIX file \"{tempPath.Replace("\"", "\\\"")}\") as «class PNGf»)";
                bool ok = RunOsascript(script, out _);
                try { System.IO.File.Delete(tempPath); } catch { }
                return ok;
            }
            catch (Exception e)
            {
                Debug.LogError($"[PhotoPopup] macOS copy to clipboard failed: {e}");
                return false;
            }
#else
            return false;
        ...
        /// <summary>
        /// 执行一段AppleScript，返回是否成功 + stdout内容。
        /// 把AppleScript写入临时.scpt文件后用osascript执行，规避命令行引号转义陷阱。
        /// </summary>
        private static bool RunOsascript(string script, out string stdout)
        {
            stdout = string.Empty;
            string scriptPath = System.IO.Path.Combine(
                System.IO.Path.GetTempPath(),
                $"featherland_{System.Guid.NewGuid():N}.applescript");
            try
            {
                ...
                using (var p = System.Diagnostics.Process.Start(psi))
                {
                    ...
                    if (p.ExitCode != 0)
                    {
                        Debug.LogWarning($"[PhotoPopup] osascript exit {p.ExitCode}: {stderr}");
                        return false;
                    }
                    return true;
                }
            ...
            {
                Debug.LogError($"[PhotoPopup] osascript run failed: {e}");
                return false;
            }
            finally
            {
...
#elif UNITY_STANDALONE_OSX
            // macOS: 用osascript的choose folder弹原生Finder对话框
            try
            {
                // AppleScript字符串中需要转义 \ 和 "
                string promptEscaped = (title ?? "").Replace("\\", "\\\\").Replace("\"", "\\\"");
                string script;
                if (!string.IsNullOrEmpty(initialDir) && System.IO.Directory.Exists(initialDir))
                {
                    string initEscaped = initialDir.Replace("\\", "\\\\").Replace("\"", "\\\"");
                    script = $"POSIX path of (choose folder with prompt \"{promptEscaped}\" default location (POSIX file \"{initEscaped}\"))";
                }
                else
                {
                    script = $"POSIX path of (choose folder with prompt \"{promptEscaped}\")";
                }
                if (RunOsascript(script, out string picked) && !string.IsNullOrEmpty(picked))
                {
                    return picked.TrimEnd('/');
                }
            }
            catch (Exception e)
            {
                Debug.LogError($"[PhotoPopup] macOS choose folder failed: {e}");
            }
#endif
            return null;
        }
