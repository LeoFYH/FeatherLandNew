...
/// </summary>
public class ClickEventDetector : MonoBehaviour
{
    /// <summary>
    /// 点击事件与目标名称的关联结构体
    ...
    public struct ClickEventItem
    {
        ...
        [Tooltip("当点击该目标时触发的事件")]
        public UnityEvent onClick; // 对应的点击事件
    }

    [Header("点击事件配置")]
    [Tooltip("UI元素的点击事件列表（通过名称匹配）")]
    public List<ClickEventItem> uiClickEvents = new List<ClickEventItem>();

    [Tooltip("3D物体的点击事件列表（通过名称匹配）")]
    public List<ClickEventItem> objectClickEvents = new List<ClickEventItem>();

    [Tooltip("点击空地时触发的事件")]
    public UnityEvent onEmptySpaceClick;
    ...
    private void CheckClickType()
    {
        ...
        // 2. 检测3D物体点击
        if (mainCamera != null && Physics.Raycast(mainCamera.ScreenPointToRay(Input.mousePosition), out RaycastHit hit))
        {
